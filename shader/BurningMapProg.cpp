	...
	, m_upper_tex(0)
{
	...
	m_lifetime = m_shader->AddUniform("u_lifetime", UNIFORM_FLOAT1);
	...
	// height map
	int tex2 = m_shader->AddUniform("u_height_map_tex", UNIFORM_INT1);
	if (tex2 >= 0) {
		float sample = 2;
		m_shader->SetUniform(tex2, UNIFORM_INT1, &sample);
	}
	// border gradient
	int tex3 = m_shader->AddUniform("u_border_gradient_tex", UNIFORM_INT1);
	if (tex3 >= 0) {
		float sample = 3;
		m_shader->SetUniform(tex3, UNIFORM_INT1, &sample);
	}
}
